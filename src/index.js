...
export default {
  async fetch(request) {
    ...
    if (pathname.startsWith("/p/")) {
      ...
      return fetch(
        `https://raw.githubusercontent.com/Loyalsoldier/clash-rules/release/${filename}`
      );
    }
    ...
    let prependObj = yaml.load(template.prepend);
    configObj = Object.assign(prependObj, configObj, appendObj);
    ...
    configObj["proxies"].forEach((proxyElem) => {
      ...
      if (name.match(/HK|hk|\u6e2f/)) {
        proxyNames.HK.push(name);
        k++;
      }
      if (name.match(/TW|tw|\u53f0/)) {
        proxyNames.TW.push(name);
        k++;
      }
      if (name.match(/SG|sg|\u72ee\u57ce|\u65b0\u52a0\u5761/)) {
        proxyNames.SG.push(name);
        k++;
      }
      if (name.match(/JP|jp|\u65e5\u672c/)) {
        proxyNames.JP.push(name);
        k++;
      }
      if (name.match(/US|us|\u7f8e\u56fd/)) {
        proxyNames.US.push(name);
        k++;
      }
      if (name.match(/Netflix|netflix|NF|NETFLIX|\u7f51\u98de/)) {
        proxyNames.NF.push(name);
        k++;
      }
      if (name.match(/gpt|GPT|Gpt|chat|Chat|CHAT/)) {
        proxyNames.CG.push(name);
        k++;
      }
      if (k === 0) {
        proxyNames.OTHERS.push(name);
      }
      if (k !== 0) {
        proxyNames.ALL.push(name);
      }
    });
    ...
    Object.keys(configObj["rule-providers"]).forEach((index) => {
      ...
      providerElem["url"] = providerElem["url"].replace(
        "_PROVIDER_PROXY|",
        providerProxy
      );
    });
